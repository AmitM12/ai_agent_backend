...
# Logging tunables
LOG_INTERIMS = _env_bool("LOG_INTERIMS", False)             # show partial transcripts
INTERIM_MIN_CHARS = int(os.getenv("INTERIM_MIN_CHARS", "8")) # min chars to print an interim
INTERIM_THROTTLE_MS = int(os.getenv("INTERIM_THROTTLE_MS", "600"))  # interim print rate-limit per stream
BYTES_TICK_MS = int(os.getenv("BYTES_TICK_MS", "1000"))      # how often to print kbps to DG
...
class DialogueLogger:
    voice_id: str
    conv: List[Tuple[str, str]] = field(default_factory=list)
    last_interim: str = ""
    last_interim_ts: float = 0.0
    bytes_to_dg: int = 0
    bytes_tick_bytes: int = 0
    bytes_tick_ts: float = field(default_factory=lambda: time.time())

    def _pfx(self) -> str:
        return f"[{self.voice_id[:8]}]"

    def note_user_interim(self, text: str):
        now = time.time()
        if not LOG_INTERIMS:
            return
        if len(text) < INTERIM_MIN_CHARS:
            return
        if text == self.last_interim and (now - self.last_interim_ts)*1000 < INTERIM_THROTTLE_MS:
            return
        self.last_interim = text
        self.last_interim_ts = now
        logger.debug(f"{self._pfx()} [U~] {text}")

    def note_user_final(self, text: str):
        self.last_interim = ""
        self.conv.append(("USER", text))
        logger.info(f"{self._pfx()} [U ▶] {text}")

    def note_bot(self, text: str):
        self.conv.append(("BOT", text))
        logger.info(f"{self._pfx()} [B ▶] {text}")

    def add_bytes(self, n: int):
        self.bytes_to_dg += n

    def maybe_tick_bytes(self):
        # print kbps roughly once per BYTES_TICK_MS
        now = time.time()
        dt = now - self.bytes_tick_ts
        if dt * 1000 >= BYTES_TICK_MS and dt > 0:
            delta = self.bytes_to_dg - self.bytes_tick_bytes
            kbps = (delta * 8.0) / 1000.0 / dt  # kilobits per second
            logger.debug(f"{self._pfx()} [DG→ASR] {delta}B / {dt:.2f}s  ≈ {kbps:.1f} kbps")
            self.bytes_tick_bytes = self.bytes_to_dg
            self.bytes_tick_ts = now
...
class CallSession:
    def __init__(self, voice_id: str):
        ...
        self.dlog: DialogueLogger = DialogueLogger(voice_id)
...
async def enablex_webhook(req: Request):
    ...
    if (payload.get("state") or "").lower() == "recognized" and payload.get("text"):
        try:
            # NEW: log final user text
            session.dlog.note_user_final(payload["text"])
    ...
    if rec_text:
        try:
            session.dlog.note_user_final(rec_text)
                ...
                try:
                    while True:
                        ...
                        if ev.get("text") is not None:
                            ...
                            if payload_b64:
                                try:
                                    ...
                                    if session:
                                        session.dlog.add_bytes(len(audio_bytes))
                                        session.dlog.maybe_tick_bytes()
                        ...
                        if ev.get("bytes") is not None:
                            ...
                            if session:
                                session.dlog.add_bytes(len(ulaw_bytes))
                                session.dlog.maybe_tick_bytes()
                            try:
                    ...
                    async for m in dg:
                        if m.type == WSMsgType.TEXT:
                            ...
                            if t == "Results":
                                ...
                                # NEW: log interims (throttled) or finals
                                if transcript and session:
                                    if is_final:
                                        session.dlog.note_user_final(transcript)
                                    else:
                                        session.dlog.note_user_interim(transcript)
...
@app.get("/debug/dialogue/{voice_id}")
async def get_dialogue(voice_id: str):
    sess = SESSIONS.get(voice_id)
    if not sess:
        raise HTTPException(status_code=404, detail="unknown voice_id")
    return JSONResponse({"voice_id": voice_id, "conversation": sess.dlog.conv})
